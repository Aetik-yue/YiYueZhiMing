...
interface BookDao {
    ...
    @Query("SELECT id, bookId, chapterIndex, title FROM chapters WHERE bookId = :bookId ORDER BY chapterIndex ASC")
    fun observeChapterMetas(bookId: String): Flow<List<ChapterMeta>>

    @Query("SELECT rawContent FROM chapters WHERE bookId = :bookId AND chapterIndex = :chapterIndex LIMIT 1")
