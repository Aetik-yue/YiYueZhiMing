...
class FortuneRepository @Inject constructor(
    ...
    suspend fun drawDailySign(date: LocalDate = LocalDate.now()): FortuneRecord {
        getToday(TYPE_SIGN, date)?.let { return it }
    ...
    suspend fun drawTarot(date: LocalDate = LocalDate.now()): FortuneRecord {
        getToday(TYPE_TAROT, date)?.let { return it }
